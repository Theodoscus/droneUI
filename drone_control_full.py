...
from djitellopy import Tello  # <-- Real Tello import
...
class RealPTello:
    """
    Wraps the djitellopy.Tello with methods expected by DroneOperatingPage:
      - is_flying boolean
      - connect, streamon, streamoff, end
      - movement commands
      - takeoff/land
    """
    ...
    def __init__(self):
        self.drone = Tello()
        self.is_flying = False  # We'll track flight state manually
...
class DroneOperatingPage(QWidget):
    ...
    A fullscreen-like UI that displays:
      ...
      - Integrates with 'video_process.run' after landing
    """
    ...
    def init_ui(self):
        try:
            ...
            # Create a notification label that you can show/hide for warnings
    ...
    def update_ui_stats(self):
        try:
            battery = self.drone.drone.get_battery()  # e.g. 0-100
            height = self.drone.drone.get_height()    # cm
            ...
            speed = self.drone.drone.get_speed()  # returns 0-100
    ...
    def update_video_stream(self):
        """
        Grab frames, convert, and scale them to the label's size.
        """
        ...
        # ---- Key step: scale pixmap to match self.stream_label size ----
        ...
        if video_path:
            try:
                ...
                QMessageBox.information(
                    self,
                    "Video Processing",
                    f"Η επεξεργασία του βίντεο ολοκληρώθηκε!\nΑποτελέσματα στον φάκελο:\n{run_folder}"
                )
            except Exception as e:
...
if __name__ == "__main__":
    ...
    field_path = "fields"  # or your chosen directory path
