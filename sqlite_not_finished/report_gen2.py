...
class DroneReportApp(QMainWindow):
    ...
    def load_results(self, flight_folder):
        ...
        # Query flight results
        query = "SELECT * FROM flight_results"
        results = pd.read_sql_query(query, conn)

        # For each ID, keep only the row with the highest confidence
        results = results.loc[results.groupby("ID")["Confidence"].idxmax()]

        # Group by ID and Class to avoid duplicates
        unique_results = results.groupby(["ID", "Class"]).first().reset_index()

        # Update chart with unique disease counts
        disease_counts = unique_results["Class"].value_counts()
        self.draw_chart(categories=disease_counts.index.tolist(), values=disease_counts.tolist())

        # Update analyzed plants and affected plants
        total_plants = results["ID"].nunique()
        affected_plants = unique_results[unique_results["Class"] != "Healthy"]["ID"].nunique()
        self.plants_analyzed_label.setText(f"Φύλλα που αναλύθηκαν: {total_plants}")
        self.affected_plants_label.setText(f"Επηρεασμένα φύλλα: {affected_plants}")

        # Count unique diseases (excluding Healthy)
        unique_diseases = unique_results[unique_results["Class"] != "Healthy"]["Class"].unique()
        self.disease_count_label.setText(f"Ασθένειες που εντοπίστηκαν: {len(unique_diseases)}")
